...
void SceneGame::LoadLevel(int level)
{
	...
	qGameObject = new ObjectsManager("Resource/map/testItem2OBJ.txt");
...
//#pragma region Xử lý simon Die

	if (player->hp <= 0 && !player->isDeath)
	{
		resetTime = 100;
		player->isDeath = true;
	}
	if (player->isDeath)
	{
		...
		else
		{
			...
			player = new Simon(revivePosition.x, revivePosition.y);
			if (gameUI->getTimer() <= 0)
				gameUI->initTimer(300);
			camera->viewport = cameraPosition;
			totalResets--;
			if (totalResets == 0)
				sceneState = EnumSceneState::Scene_End;
		}
	}
